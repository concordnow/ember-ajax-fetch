...
export async function parseJSON(response) {
  ...
  console.log(`[DEBUG] ${traceId}: Content-Type header =`, response.headers.get('content-type'));
  ...
  return new Promise((resolve) => {
    if (responseType.includes('json')) {
      ...
      return response
        .json()
        .then((json) => {
          console.log(`[DEBUG] ${traceId}: JSON parsing successful, data =`, JSON.stringify(json));
          if (response.ok) {
            console.log(`[DEBUG] ${traceId}: Resolving successful JSON response`);
            return resolve({
              status: response.status,
              ok: response.ok,
              json,
        ...
        .catch((err) => {
          console.log(`[DEBUG] ${traceId}: JSON parsing failed, error =`, err.message);
      ...
      return response
        .blob()
        .then((blob) => {
          console.log(`[DEBUG] ${traceId}: Blob processing successful, size =`, blob.size);
          return resolve({
            status: response.status,
            ok: response.ok,
            blob,
        ...
        .catch((err) => {
          console.log(`[DEBUG] ${traceId}: Blob processing failed, error =`, err.message);
      ...
      return response
        .text()
        .then((text) => {
          console.log(`[DEBUG] ${traceId}: Text processing successful, length =`, text.length);
          return resolve({
            status: response.status,
            ok: response.ok,
            text,
        ...
        .catch((err) => {
          console.log(`[DEBUG] ${traceId}: Text processing failed, error =`, err.message);
          handleError(error, err, traceId);
...
function handleError(error, err, traceId) {
  console.log(`[DEBUG] ${traceId}: handleError: Processing error, original error =`, error, 'caught error =', err);
  if (isJsonString(error.message)) {
    error.payload = JSON.parse(error.message);
    console.log(`[DEBUG] ${traceId}: handleError: Parsed JSON payload =`, error.payload);
  } else {
    error.payload = error.message || err.toString();
    console.log(`[DEBUG] ${traceId}: handleError: Non-JSON payload =`, error.payload);
  }
  error.message = error.message || err.toString();
  console.log(`[DEBUG] ${traceId}: handleError: Final processed error =`, error);
}
